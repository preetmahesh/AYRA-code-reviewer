...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('ayra.sidebar', provider)
  );
}

export function deactivate() {}

class AyraSidebarProvider implements vscode.WebviewViewProvider {
  ...
  private async _handleMessage(userText: string, mode: string) {
  ...
  private async _applyFix(code: string) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
  ...
  private async _undoFix() {
    const editor = vscode.window.activeTextEditor;
    if (!editor || !this._snapshot) return;
